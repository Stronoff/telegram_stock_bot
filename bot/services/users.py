...
async def update_user_token(session: AsyncSession, user_id: int, tinkoff_token: str) -> None:
    stmt = update(UserModel).where(UserModel.id == user_id).values(tinkoff_token=tinkoff_token)
    await session.execute(stmt)
    await session.commit()


async def update_user_account_id(session: AsyncSession, user_id: int, account_id: int) -> None:
    stmt = update(UserModel).where(UserModel.id == user_id).values(account_id=int(account_id))
    await session.execute(stmt)
    await session.commit()
